...
try:
    ...
    st.error("Woi Bre, file config.yaml belum lu bikin! Cek chat sebelumnya.")
    st.stop()
...
# --- 3. GERBANG KEAMANAN UTAMA ---
if st.session_state.get("authentication_status"):
    
    # --- KODINGAN ASLI LU DIMULAI DARI SINI ---
    ...
    # --- BACKEND FUNCTIONS (G-DRIVE & KURS) ---
    ...
    def sync_data():
        try:
            ...
            FAISS.from_documents(chunks, GoogleGenerativeAIEmbeddings(model="gemini-embedding-001", google_api_key=API_KEY)).save_local("db_ingatan_faiss")
    ...
    # --- SIDEBAR: CHAT HUB ---
    with st.sidebar:
        # Tombol Logout ditampilin paling atas di sidebar
        st.write(f"Selamat datang, **{st.session_state['name']}** 🛡️")
        authenticator.logout('Keluar (Logout)', 'main')
        st.divider()
        ...
        with st.chat_message("assistant"):
            with st.spinner("Mencari di memori..."):
                if not os.path.exists("db_ingatan_faiss"):
                    ...
                    # Gue tambahin google_api_key di sini biar lu gak kena error 401 lagi ke depannya
...
# --- 4. LOGIKA JIKA GAGAL LOGIN ---
    ...
    st.warning("🔒 Silakan masukin kredensial lu buat buka Database G-Drive.")